...
def parse_dataset():
    ...
    for ds_year in ds_years:
        ...
        for idx_month, ds_year_month in enumerate(ds_year_months):
            ...
            if (idx_month + 1) == 6:
                df_yr.to_csv(
                    f"/data/logs-datasets/kyoto-2016/labeled/{prefix}kyoto-2016_{ds_year}_p1.csv")
                del df_yr
                df_yr = pd.DataFrame()
            if (idx_month + 1) == 12:
                df_yr.to_csv(
                    f"/data/logs-datasets/kyoto-2016/labeled/{prefix}kyoto-2016_{ds_year}_p2.csv")

        if ds_year == "2006":
            df_yr.to_csv(
                f"/data/logs-datasets/kyoto-2016/labeled/{prefix}kyoto-2016_{ds_year}.csv")
        del df_yr
...
def split_iid(p_test=0.1, random_state=10):
    for year in ds_years:
        df_year = pd.read_pickle(
            f"/data/logs-datasets/kyoto-2016/{prefix}kyoto-2016_{year}.pkl")
        ...
        train_path = f"/data/logs-datasets/kyoto-2016/{prefix}iid_kyoto-2016_{year}_train.pkl"
        test_path = f"/data/logs-datasets/kyoto-2016/{prefix}iid_kyoto-2016_{year}_test.pkl"
