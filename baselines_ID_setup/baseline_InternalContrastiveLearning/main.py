...
def main(dataset, db_path, full_set):
    args.dataset = dataset 
    
    trainer_object=trainer(args)
    
    f_score,auc_score,pr_auc_in, pr_auc_out,df_=trainer_object.train_and_evaluate(db_path, full_set)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--full_set', type=int, default=1, help='full set or subset')
    args = parser.parse_args()
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    
    dataset = args.dataset
    db_path = args.dataset_path
    _, auc, pr_auc_in, pr_auc_out = main(dataset, db_path, args.full_set)
